...
function Chat() {
	...
	return (
		<div className="h-screen bg-slate-100 dark:bg-gradient-to-r dark:from-black-600 dark:via-black-700 dark:to-black-600">
			...
			{/* Main Chat Area */}
			<main
				className={`h-full pb-10 pt-8 flex flex-col transition-all relative ${user && isSidebarOpen ? 'md:ml-80' : 'ml-0'}`}
			>
				{/* Chat Header */}
				<header className="z-4 -mt-8 pt-2 flex items-center justify-between bg-transparent">
					<div
						className={`flex items-center justify-start min-h-14 mt-2 z-10 transition-all duration-300 ease-in-out ${user && isSidebarOpen ? ' translate-x-0 ml-2' : ' -translate-x-1 ml-6'}`}
					>
						{/* Sidebar button */}
						{!isSidebarOpen && user && (
							<SidebarButton
