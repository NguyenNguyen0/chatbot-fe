...
function ChatSidebar({ isOpen, onToggle, onOpenDeleteDialog, className }) {
	...
	return (
		<>
			{/* Sidebar */}
			<aside
				className={`fixed z-10 top-0 bottom-0 left-0 flex flex-col h-screen bg-slate-100 dark:bg-black-700 transition-transform duration-300 transform ${isOpen ? 'w-80 translate-x-0' : 'w-80 -translate-x-full'} ${className}`}
			>
							...
							<div className="relative flex items-center justify-center">
								...
								{openMenuId === conversation.chatId && (
									<DropdownMenu
										...
										onRename={() =>
											startRenameConversation(conversation.chatId)
										}
				...
				<div className="py-2 px-3">
					<button
						className="flex items-center p-3 justify-start gap-3 cursor-pointer hover:bg-black-200 dark:hover:bg-black-600"
			...
			{/* Overlay */}
			{isOpen && (
				<div className="fixed inset-0 bg-black/20 z-9 md:hidden" onClick={onToggle} />
			)}
		</>
	);
}
